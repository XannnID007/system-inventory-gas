...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <a href="{{ route('transaksi.create') }}" class="px-6 py-3 gradient-purple text-white font-bold rounded-xl hover:shadow-lg hover:shadow-purple-500/50 transition-all flex items-center gap-2">
    ...
    <div class="glass-white rounded-2xl p-6 shadow-lg">
        <form action="{{ route('transaksi.index') }}" method="GET" class="flex flex-col md:flex-row gap-4">
            ...
            <div class="flex gap-2">
                <button type="submit" class="px-6 py-3 gradient-purple text-white font-semibold rounded-xl hover:shadow-lg transition-all">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                    </svg>
                </button>
                <a href="{{ route('transaksi.index') }}" class="px-6 py-3 bg-gray-200 text-gray-700 font-semibold rounded-xl hover:bg-gray-300 transition-all">
                    Reset
                </a>
            </div>
        </form>
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-gradient-to-r from-purple-50 to-pink-50">
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">No Invoice</th>
                        ...
                        <th class="px-6 py-4 text-center text-xs font-bold text-gray-700 uppercase tracking-wider">Metode</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-purple-50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <p class="font-semibold text-gray-900">{{ $item->tanggal_transaksi->format('d/m/Y') }}</p>
                        ...
                        <td class="px-6 py-4 text-right">
                            <p class="font-bold text-gray-900 text-lg">Rp {{ number_format($item->total, 0, ',', '.') }}</p>
                            @if($item->diskon > 0)
                            <p class="text-xs text-green-600">Diskon: Rp {{ number_format($item->diskon, 0, ',', '.') }}</p>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-center">
                            ...
                            @elseif($item->metode_bayar === 'transfer')
                                <span class="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-bold inline-flex items-center gap-1">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            @else
                                <span class="px-3 py-1 bg-purple-100 text-purple-700 rounded-full text-xs font-bold inline-flex items-center gap-1">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('transaksi.show', $item->id) }}" class="p-2 bg-blue-100 hover:bg-blue-200 text-blue-600 rounded-lg transition-colors" title="Detail">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                    </svg>
                                </a>
                                <form action="{{ route('transaksi.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus transaksi ini? Stok akan dikembalikan.')">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center">
                            ...
                            <a href="{{ route('transaksi.create') }}" class="inline-flex items-center gap-2 px-6 py-3 gradient-purple text-white font-semibold rounded-xl hover:shadow-lg transition-all">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                                </svg>
                                Buat Transaksi
